...
void rek(int n, int d, int* niza, int* krajna, int c, int b) {
    for (int i = 1; i < n; i++) {
        int sega_novo = rastojanie(niza[c*2], niza[c*2+1], niza[i*2], niza[i*2+1]);
        int sega_kraj = rastojanie(niza[c*2], niza[c*2+1], niza[n*2-2], niza[n*2-1]);
