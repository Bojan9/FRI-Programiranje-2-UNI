...
// Recursive function to calculate the optimal score difference
int optimalScoreDifference(int* nums, int start, int end) {
    if (start == end) {
        return nums[start];
    }

    int chooseStart = nums[start] - optimalScoreDifference(nums, start + 1, end);
    int chooseEnd = nums[end] - optimalScoreDifference(nums, start, end - 1);

    return chooseStart > chooseEnd ? chooseStart : chooseEnd;
}

// Main function to predict the winner
bool predictTheWinner(int* nums, int numsSize) {
    // If the number of elements is even, Player 1 always wins
    if (numsSize % 2 == 0) {
        return true;
    }

    // Calculate the optimal score difference recursively
    return optimalScoreDifference(nums, 0, numsSize - 1) >= 0;
}
